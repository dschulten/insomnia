...
import { updateMimeType, update } from '../../models/request';
...
class App extends PureComponent {
  ...
  _pushToHistory() {
    ...
    const historyDescriptor = activeRequest.history;
    const url = activeResponse.url;

    if (historyDescriptor.current === historyDescriptor.responses.length - 1) {
      const mostRecent = historyDescriptor.responses[historyDescriptor.responses.length - 1];
      if (!mostRecent || mostRecent.url !== url) {
        let historyRequest;
        historyRequest = models.request.newRequest(url, 'GET', '');
        historyDescriptor.responses.push(historyRequest);
  ...
  _handleHistoricRequest(activeRequestId, activeEnvironmentId, historyDescriptor, isBack) {
    let request = historyDescriptor.responses[historyDescriptor.current];
    let requested = historyDescriptor.requested[historyDescriptor.current];
    if (request.url === requested.url && requested.method !== 'GET') {
      ...
      return this._handleSendRequestWithEnvironment(activeRequestId, activeEnvironmentId, request);
    }
  }
